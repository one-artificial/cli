...
fn all_commands(project_dir: &str) -> Vec<Suggestion> {
    let mut cmds = vec![
        ...
        cs("/evergreen", "Toggle background context compression"),
        cs("/chronicle", "Toggle cross-session synthesis"),
        cs("/prelude", "Toggle speculative pre-computation"),
        cs("/calibrate", "Toggle skill improvement analysis"),
        cs("/palimpsest", "Toggle living doc maintenance"),
